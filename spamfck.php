<?php
	...
	Description: Antispam protection:<br><strong>1.</strong> Add 3 hidden fields to Registration form and to Comments form, that only robots can see. Plus you can choose to add validation checkbox to forms.<br><strong>32</strong> Add to .htaccess file direct access protection for wp-comments-post.php file.<br><strong>3.</strong> Check time user spent on page and time when user send a form, if it´s under time limit (5s on registration form, 10s on comment form).
	Version: 1.2
...
// --THIRD WAY OF PROTECTION - COMPARE TIME USER SPENT ON THE PAGE AND TIME WHEN HE SEND A COMMENT
 ...
 $spamfckstats_update = $spamfckstats+1;        //Spam statistics update
    ...
    if ( !empty( $_POST['message'] ) )
            ...
            update_option( 'spamfck_stats', $spamfckstats_update, 'yes' );
    if ( !empty( $_POST['description'] ) )                                     //Spam statistics update
